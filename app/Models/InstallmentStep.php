...
class InstallmentStep extends Model implements TranslatableContract
{
    ...
    public function getDeadlineTitle($itemPrice = 1,$itemId=null)
    {
        ...
        if(!empty($itemId))
        {
            ...
            return trans('update.amount_after_n_days', ['amount' => handlePrice($this->getPrice($itemPrice))." تاريخ استحقاق ".$this->title, 'days' => dateTimeFormat(($this->installment->deadline_type == 'days') ? (($this->deadline * 86400) + $bundle->start_date) : $this->deadline , 'j M Y'), 'percent' => $percentText]);
