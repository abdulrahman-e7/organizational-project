...
class UserController extends Controller
{
    ...
    public function getLoggedInUserStudentData(Request $request)
    {
        ...
        if (!$student) {
            return apiResponse2(0, 'not_found', trans('api.public.student_not_found'), []);
        }
                ...
                foreach ($meetingTime as $time) {
                    ...
                    if ($user) {
                        $userReservedMeeting = ReserveMeeting::where('user_id', $user->id)
                            ->where('meeting_id', $meeting->id)->where(
                                'meeting_time_id',
                                $time->id
                            )
                            ->first();
