...
use App\Http\Controllers\PaytabsPageController;
...
Route::get('/geo-ip-route', function(){    
    $cityDbReader = new Reader(storage_path('app/geoip/GeoLite2-Country.mmdb'));
    $record = $cityDbReader->country("196.141.104.21");     # This should be dynamic - request()->ip() in production environment. 192.158.205.178 | 217.12.220.0
    print($record->country->isoCode . "<br />"); 
    print($record->country->name . "<br />"); 
    print($record->country->names['zh-CN'] . "<br />");
    print($record->traits->network . "<br />");    
});

Route::get('/try', function(Request $request){  
    
    dd($request->get('countryCurrency'));
})->middleware('vpn');


// paymob
Route::get('/paymob-integration/pay', [CreditController::class, 'viewForm'])->name('pay');
Route::post('/paymob-integration/credit', [CreditController::class, 'credit'])->name('credit');
Route::get('/paymob-integration/callback', [CreditController::class, 'callback'])->name('callback');

// paytabs
Route::get('/paytabs/pay', [PaytabsPageController::class, 'index']);
